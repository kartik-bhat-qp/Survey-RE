...
export default function TextAiPage() {
  ...
  function handleCreate({
    ...
    const creationQuestions = questionIds.map((questionId, index) => {
      const question = getTextAiQuestionById(questionId);
      return {
        code: question?.code ?? `Q${index + 1}`,
        context: questionContexts[questionId]?.trim() ?? '',
        text: question?.text ?? `Question ${index + 1}`,
      };
    });
    const codebookPreference =
      codebookSource === 'template'
        ? 'Uploaded from a codebook template file'
        : codebookSource === 'report'
          ? `Selected from existing${
              reportCodebook?.name ? ` · ${reportCodebook.name}` : ''
            }`
          : 'Generated by QuestionPro AI';
    const baseCreationPreferences = {
      codebookPreference,
      codebookFileDataUrl:
        codebookSource === 'template' ? codebookFileDataUrl : undefined,
      codebookFileName:
        codebookSource === 'template'
          ? codebookFileName || 'TextAI codebook template.xlsx'
          : undefined,
      dashboardLayout: separateDashboardPerQuestion
        ? ('Separate dashboard for each question' as const)
        : ('Combined dashboard' as const),
      dataSourceName: survey.name,
      dataSourceType: 'Surveys',
      outputLanguage: outputLanguage.label,
      questions: creationQuestions,
      themeModelingPrompt: modelingGoal.trim() || 'Not provided',
    };
    const createdDashboards: TextAiDashboard[] = separateDashboardPerQuestion
      ? questionIds.map((questionId, index) => {
          ...
          return {
            ...
            segmentFilters,
            creationPreferences: {
              ...baseCreationPreferences,
              questions: [creationQuestions[index]],
            },
          };
        })
      : [
          {
            ...
            segmentFilters,
            creationPreferences: baseCreationPreferences,
          },
        ];
